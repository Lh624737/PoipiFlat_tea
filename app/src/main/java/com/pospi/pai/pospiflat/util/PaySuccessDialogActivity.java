...
public class PaySuccessDialogActivity extends PayBaseActivity {
    ...
    public final static String PUT_MTYPE= "type";
...
//                .get(whichOne).getName();
        ...
        String shopName = getSharedPreferences("shopMsg", MODE_PRIVATE).getString("name", "");

        payway = getIntent().getStringExtra(PUT_PAYWAY);
        maxNo = getIntent().getStringExtra(PUT_MAXNO);
        orderType = getIntent().getIntExtra(PUT_MTYPE,URL.ORDERTYPE_SALE);
...
//        }
    ...
    public void printPaper(View v) {
        if (v.getId() == R.id.btn_print) {
            new Printer().print(this ,ss,payway ,maxNo ,valueCardDto ,sid ,orderType );
