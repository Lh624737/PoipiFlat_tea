...
public class MaxNO {
    ...
    public static String getMaxNo(Context context) {
        String jiju = context.getSharedPreferences("receipt_num", Context.MODE_PRIVATE).getString("pay_jiju","01");
        String maxNo;
        List<OrderDto> orderDtos = new OrderDao(context).findMaxNO(GetData.getYYMMDDTime());

        if (orderDtos.size() == 0) {
            maxNo =jiju+GetData.getYYMMDD()+ "0001";
            ...
            Log.i("no", number + ">>>>>>>>>");
            maxNo = jiju + GetData.getYYMMDD() + String.format("%04d", Integer.parseInt(number)+1);
            Log.i("no", maxNo + ">>>>>>>>>");
        }
