...
//            Order.put("shopNo", orderDto.getShop_id());//店铺号
    ...
    public void uploadOrderToServer(final OrderDto orderDto, PayWayDto payWayDto, String orderType, final Context context, final int from) {
        ...
        //得到店铺的Id
        SharedPreferences preferences = context.getSharedPreferences("shopMsg", MODE_PRIVATE);
        String shopid = preferences.getString("shopId", "");//得到店铺的ID
        SharedPreferences ssp = context.getSharedPreferences("receipt_num", Context.MODE_PRIVATE);

        //得到当前的时间
        final String date = GetData.getDataTime();
        //得到就餐的人数
        double paxNumber = 0;
//        preferences = context.getSharedPreferences("Token", Context.MODE_PRIVATE);
//        String value = preferences.getString("value", "");
//        String[] names = value.split(",");
        String deviceNo = ssp.getString("pay_jiju","");//收银机号
//        String uid = names[1];
...
//        String by = cashierMsgDtos.get(whichOne).getNumber();//得到收银员的账号
        ...
        String by = sp.getString("cashierName", "1000");
//        String by = cashierMsgDtos.get(whichOne).getNumber();//得到收银员的账号
        JSONObject Order = new JSONObject();
...
//        jsonArrayOrder_PayType.put(Order_PayType);
        ...
        try {
            ...
            double card_discount = 1;
            if (orderDto.getPayway().equals(PayWay.CZK)) {
                card_discount = context.getSharedPreferences("StoreMessage", MODE_PRIVATE).getFloat("Discount", 1);
            }
            //订单详情
            for (int i = 0; i < goodsDtos.size(); i++) {
                GoodsDto goodsDto = goodsDtos.get(i);
...
//                jsonArrayOrder_Menus.put(Order_Menus);
                ...
                json.put("zp", 1);//赠品(1非赠品/2赠品)
                json.put("sl", goodsDto.getNum());//销售数量
                json.put("sj",DoubleSave.doubleSaveTwo((goodsDto.getPrice()*goodsDto.getNum()-goodsDto.getDiscount()))/goodsDto.getNum());//零售价
                json.put("xsj" ,DoubleSave.doubleSaveTwo(goodsDto.getPrice()*goodsDto.getNum()-goodsDto.getDiscount()));//销售价
                json.put("zzk", goodsDto.getDiscount());//总折扣
                json.put("hyzk", "");//会员折扣
                json.put("popzk", "");//促销折扣
                json.put("lszk", "");//临时折扣
                json.put("syje", "");//损益金额
                json.put("vipid", orderDto.getVipId());//会员卡
                json.put("vipjf", "");//积分
                json.put("vipje", "");//零钱余额
                json.put("popid", goodsDto.getPopid());//促销单id
                json.put("ispop", goodsDto.getIspop());
                json.put("iszp", goodsDto.getIszp());
                json.put("poptype", goodsDto.getPoptype());
                array.put(json);
            }
            head.put("market", shopid);//门店号
            head.put("fphm", orderDto.getMaxNo());//小票号
            head.put("syjh", deviceNo);//收银机号
            head.put("syy", context.getSharedPreferences("syy", MODE_PRIVATE).getString("countNum","100"));//收银员
            head.put("yyy", "");//营业员
            head.put("sjje", "");//零售价金额
            ...
            head.put("hyzk", 0);//会员折扣
            head.put("popzk", 0);//促销折扣
            head.put("lszk", 0);//临时折扣
            head.put("syje", 0);//损益金额
            head.put("vipid", orderDto.getVipId());//会员卡
            head.put("vipjf", "");//积分
            head.put("type",orderType);//订单类型
            head.put("vipje", "");//零钱余额
            head.put("sendrq", orderDto.getCheckoutTime());//发送日期

            for (int i =0 ;i<orderPaytypes.size();i++) {
                JSONObject ob = new JSONObject();
                ob.put("payid", orderPaytypes.get(i).getPayCode());
                ob.put("payje", orderPaytypes.get(i).getYs());
                pay.put(ob);
            }

            order.put("header", head);
            order.put("pay", pay);
            ...
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                if (i == 200) {
                    ...
                    try {
                        ...
                        returnDto.setCode(Integer.parseInt(object.getString("code")));
//                        returnDto.setMessage(object.getString("Message"));
//                        returnDto.setValue(String.valueOf(object.getString("Value")));
    ...
    private void kucunUp(String a ,String b) {

        RequestParams params = new RequestParams();
        params.put("billid", a);
        params.put("billtype", b);
        new Server().getConnect(context, new URL().SYNCORDER_KC, params, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                Log.i("url", i + ">>>>>>"+new String(bytes));
            }
 ...
 //            Order.put("shopNo", orderDto.getShop_id());//店铺号
    ...
    public void postWebServer(final OrderDto orderDto, String payWay, boolean sale) {
        //得到店铺的Id
        String Shopid = context.getSharedPreferences("StoreMessage", MODE_PRIVATE).getString("Id", "");//得到店铺的ID
        String Response = context.getSharedPreferences("cashierMsgDtos", MODE_PRIVATE).getString("cashierMsgDtos", "");
        List<CashierMsgDto> cashierMsgDtos = new CashierLogin_pareseJson().parese(Response);
        int whichOne = context.getSharedPreferences("islogin", MODE_PRIVATE).getInt("which", 0);
        String cashier = cashierMsgDtos.get(whichOne).getNumber();//得到收银员的账号

        SharedPreferences preferences = context.getSharedPreferences("ERP", MODE_PRIVATE);
...
//                                Toast.makeText(context, "ERP上传成功！", Toast.LENGTH_SHORT).show();
    ...
    public void upCardRecord(String sid, OrderDto orderDto, ValueCardDto valueCardDto, int orderType) {
        ...
        try {
            ...
            preferences = context.getSharedPreferences("StoreMessage", MODE_PRIVATE);
            String Shopid = preferences.getString("Id", "");//得到店铺的ID
