...
public class PaymentAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (payType==(PayWay.CASH)) {//现金
            ...
            holder.iv_payment.setImageResource(R.mipmap.icon_lq);
            holder.tv_payment.setText(payWayDto.getName());
        } else if (payType==(PayWay.E_COUPON)) {//电子券
            holder.iv_payment.setImageResource(R.drawable.icon_chuzhika);
            holder.tv_payment.setText(payWayDto.getName());
        } else if (payType==(PayWay.pay_zp)) {//支票
            holder.iv_payment.setImageResource(R.mipmap.icon_zp);
            holder.tv_payment.setText(payWayDto.getName());
        } else if (payType==(PayWay.pay_lnk)) {//老年卡
            holder.iv_payment.setImageResource(R.mipmap.icon_lnk);
            holder.tv_payment.setText(payWayDto.getName());
        }else {
            holder.iv_payment.setImageResource(R.drawable.icon_other);
            holder.tv_payment.setText(payWayDto.getName());
