    ...
    public void addOrder(OrderDto orderDto) {
       ...
       // String sid = orderDto.getOrderSid();
        db.execSQL("insert into orderinfo(maxNo,orderType,payway,shop_id,time,ss_money,ys_money,zl_money,order_info," +
                        ...
                        "serialNumber,tableNumber,eatingNumber,orderId,upLoadServer,upLoadERP,orderNo,miYaNumber,orderSid,vipId)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                new Object[]{orderDto.getMaxNo(), orderDto.getOrderType(), orderDto.getPayway(), orderDto.getShop_id(), orderDto.getTime(), orderDto.getSs_money(), orderDto.getYs_money(), orderDto.getZl_money(), orderDto.getOrder_info(),
                        ...
                        orderDto.getStatus(), orderDto.getOrderId(), orderDto.getUpLoadServer(), orderDto.getUpLoadERP(),orderDto.getOrderNo(),orderDto.getMiYaNumber(),orderDto.getOrderSid() ,orderDto.getVipId()});
        db.close();
//        Log.i("addOrder",""+orderDto.getUpLoadERP());
    ...
    public List<OrderDto> selectGoods(String time) {
        String shopId = context.getSharedPreferences("userMsg", Context.MODE_PRIVATE).getString("uid", "");
...
//            dto.setMiYaNumber(cursor.getString(cursor.getColumnIndex("miYaNumber")));
            dto.setVipId(cursor.getString(cursor.getColumnIndex("vipId")));
            dtosss.add(dto);
        }
        cursor.close();
        db.close();
...
//            dto.setMiYaNumber(cursor.getString(cursor.getColumnIndex("miYaNumber")));
    ...
    public List<OrderDto> selectRestOrder() {
        ...
        List<OrderDto> dtosss = new ArrayList<>();
        Cursor cursor = db.query(tablename, null, "time=? and shop_id=? and ifFinishOrder=?", new String[]{time, shopId, String.valueOf(URL.BAOLIU)}, null, null, null);
        while (cursor.moveToNext()) {
            dto = new OrderDto();
            //遍历Cursor对象，取出数据并打印
            dto.setMaxNo(cursor.getString(cursor.getColumnIndex("maxNo")));
