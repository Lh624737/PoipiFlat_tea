    ...
    public void addGoods(List<GoodsDto> dtos) {
        ...
        try {
            ...
            for (int i = 0; i < dtos.size(); i++) {
                ...
                values.put("Sid", goodsDto.getGoods_id());
...
//        Log.i("删除表格数据", "删除数据成功");
    ...
    public GoodsDto selectGoods(String code) {
        ...
        if (cursor.moveToFirst()) {
            ...
            dto.setSpecification(cursor.getString(cursor.getColumnIndex("specification")));
    ...
    public GoodsDto selectGoodsById(String id) {
        SQLiteDatabase db = goodsDB.getReadableDatabase();

        Cursor cursor = db.query(tablename, null, "Sid=?", new String[]{id}, null, null, null);
        if (cursor.moveToFirst()) {
            dto = new GoodsDto();
            //遍历Cursor对象，取出数据并打印
            dto.setName(cursor.getString(cursor.getColumnIndex("name")));
    ...
    public List<GoodsDto> findSelectPointGoods() {
        ...
        while (cursor.moveToNext()) {
            ...
            Log.i("setSid", "code: "+cursor.getString(cursor.getColumnIndex("Code")));
